...
extends 
  ...
  return Scaffold(body: BlocListener<MovieWatchlistBloc, MovieWatchlistState>(listener: (_, state) {
   ...
   child: CircularProgressIndicator(),);} 
         ...
         bool isAddedToWatchlist = (context.watch<MovieWatchlistBloc>().state is MovieWatchlistStatusLoaded) 
         ? (context.read<MovieWatchlistBloc>().state as MovieWatchlistStatusLoaded)  .result : false;
        return SafeArea(
   child: DetailContent(
         movie,
...
class DetailContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
   ...
   placeholder: (context, url) => Center(
    ...
    Container( margin: const EdgeInsets.only(top: 48 + 8),
    ...
    controller: scrollController,
     ...
     children: [Text(movie.title,style: kHeading5,),
      ElevatedButton(onPressed: () async {
     if (!isAddedWatchlist) {

     BlocProvider.of<MovieWatchlistBloc>(context)..add(AddItemMovieEvent(movie));
     } else {
     BlocProvider.of<MovieWatchlistBloc>(context) ..add(RemoveItemMovieEvent(movie)); 
    }
 },
     child: Row(mainAxisSize: MainAxisSize.min,
      children: [isAddedWatchlist  ? Icon(Icons.check) : Icon(Icons.add),
      Text ('Watchlist'), ],),),
      Text(_showGenres(movie.genres),  ),
      Text(_showDuration(movie.runtime), ),
      Row( children: [RatingBarIndicator( rating: movie.voteAverage / 2,   
      itemCount: 5, itemBuilder: (context, index) => Icon(Icons.star,
      color: kMikadoYellow, ), itemSize: 24, ),
      ...
      BlocBuilder<MovieRecommendationBloc,
       MovieRecommendationState>( builder: (context, state) {
      ...
      itemBuilder: (context, index) {final movie = recommendations[index];
       return Padding(
      padding: const EdgeInsets.all(4.0),
       child: InkWell( onTap: () { Navigator.pushReplacementNamed(context, MovieDetailPage.ROUTE_NAME,arguments: movie.id,
     );
 },
       ...
       child: ClipRRect(borderRadius: BorderRadius.all( Radius.circular(8), ),
            child: CachedNetworkImage( imageUrl: 'https://image.tmdb.org/t/p/w500${movie.posterPath}',
             ...
             return Container();
                ...
                Align(alignment: Alignment.topCenter, child: Container( color: Colors.white, height: 4,  width: 48,
               ),
           ),
        ],
     ),
        ...
        Padding(padding: const EdgeInsets.all(8.0),
          child: CircleAvatar(backgroundColor: kRichBlack,
